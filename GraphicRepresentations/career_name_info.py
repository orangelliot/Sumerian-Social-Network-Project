...
# SELECT DISTINCT rawnames.tabid, yearsequence.year, yearsequence.sequence
...
# 	WHERE rawnames.name = 'en-mah-gal-an-na';
...
for name in names:
	...
	query = f"SELECT DISTINCT rawnames.tabid, yearsequence.year, yearsequence.sequence FROM rawnames INNER JOIN bestyears ON rawnames.tabid = bestyears.tabid INNER JOIN yearsequence ON bestyears.year = yearsequence.year WHERE rawnames.name = '{name}';"
	# Query the database
...
for i in range(name_count):
	...
	bar_plt.set_xlabel("Sequential Year")
...
for i in range(name_count):
	...
	careerPlot.scatter(years, np.ones(len(years)) * i, label=names[i], marker='|', s=scales)
...
careerPlot.set_xlabel("Relative Linear Date")
